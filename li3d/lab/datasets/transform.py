...
def index_operator(data_dict, index, duplicate=False, keys=None, keys_key="index_valid_keys"):
    """
    keys_key : 用哪组 key 列表来切片
               "index_valid_keys"           -> 原始流
               "index_valid_keys_trans"     -> 变换流
    """
    # 1. 确定要切片的 key 列表
    if keys is None:
        keys = data_dict.get(keys_key, [])  # 用指定的那一组
    if duplicate:
...
class PointClip(object):
    def __init__(self, point_cloud_range=(-80, -80, -3, 80, 80, 1), coord_key="coord", label_key=None):
        self.point_cloud_range = point_cloud_range
        self.coord_key = coord_key
        self.label_key = label_key

    def __call__(self, data_dict):
        if self.coord_key in data_dict.keys():
            data_dict[self.coord_key] = np.clip(
                data_dict[self.coord_key],
                a_min=self.point_cloud_range[:3],
                a_max=self.point_cloud_range[3:],
            )
            if self.label_key in data_dict.keys():
                mask = np.all(
                    (data_dict[self.coord_key] >= self.point_cloud_range[:3]) &
                    (data_dict[self.coord_key] <= self.point_cloud_range[3:]),
                    axis=1
                )
                data_dict[self.coord_key] = data_dict[self.coord_key][mask]
                data_dict[self.label_key] = data_dict[self.label_key][mask]
        return data_dict


@TRANSFORMS.register_module()
class RandomDropout(object):
    def __init__(self, dropout_ratio=0.2, dropout_application_ratio=0.5, coord_key="coord", label_key=None,
                 sampled_index_key="sampled_index", segment_key="segment", index_valid_keys=None,
                 index_valid_keys_key="index_valid_keys"):
        self.dropout_ratio = dropout_ratio
        self.dropout_application_ratio = dropout_application_ratio
        self.coord_key = coord_key
        self.label_key = label_key
        self.sampled_index_key = sampled_index_key
        self.segment_key = segment_key
        self.index_valid_keys = index_valid_keys
        self.index_valid_keys_key = index_valid_keys_key

    def __call__(self, data_dict):
        if random.random() < self.dropout_application_ratio:
            n = len(data_dict[self.coord_key])
            idx = np.random.choice(n, int(n * (1 - self.dropout_ratio)), replace=False)
            if self.sampled_index_key in data_dict:
                idx = np.unique(np.append(idx, data_dict[self.sampled_index_key]))
                mask = np.zeros_like(data_dict[self.segment_key]).astype(bool)
                mask[data_dict[self.sampled_index_key]] = True
                data_dict[self.sampled_index_key] = np.where(mask[idx])[0]
            data_dict = index_operator(data_dict, idx, keys=self.index_valid_keys, keys_key=self.index_valid_keys_key)
            if self.label_key in data_dict.keys():
                data_dict[self.label_key] = data_dict[self.label_key][idx]
        return data_dict
    ...
    def __call__(self, data_dict):
        ...
        if self.coord_key in data_dict.keys():
            if self.center is None:
                x_min, y_min, z_min = data_dict[self.coord_key].min(axis=0)
                x_max, y_max, z_max = data_dict[self.coord_key].max(axis=0)
                center = [(x_min + x_max) / 2, (y_min + y_max) / 2, (z_min + z_max) / 2]
            else:
                center = self.center
            data_dict[self.coord_key] -= center
            data_dict[self.coord_key] = np.dot(data_dict[self.coord_key], np.transpose(rot_t))
            data_dict[self.coord_key] += center
        if self.normal_key in data_dict.keys():
            data_dict[self.normal_key] = np.dot(data_dict[self.normal_key], np.transpose(rot_t))
        if self.label_key in data_dict.keys():
            if self.label_key == "transform_matrix":
                data_dict[self.label_key][:3, :3] = np.dot(rot_t, data_dict[self.label_key][:3, :3])
        return data_dict
...
class RandomScale(object):
    def __init__(self, scale=None, anisotropic=False, coord_key="coord", label_key=None):
    ...
    def __call__(self, data_dict):
        if self.coord_key in data_dict.keys():
            ...
            data_dict[self.coord_key] *= scale
            if self.label_key in data_dict.keys():
                if self.label_key == "transform_matrix":
                    if self.anisotropic:
                        data_dict[self.label_key][:3, :3] = np.diag(scale) @ data_dict[self.label_key][:3, :3]
                    else:
                        data_dict[self.label_key][:3, :3] *= scale
        return data_dict


@TRANSFORMS.register_module()
class RandomFlip(object):
    def __init__(self, p=0.5, coord_key="coord", normal_key="normal", label_key=None):
        self.p = p
        self.coord_key = coord_key
        self.normal_key = normal_key
        self.label_key = label_key
    ...
    def __call__(self, data_dict):
        if np.random.rand() < self.p:
            if self.coord_key in data_dict.keys():
                data_dict[self.coord_key][:, 0] = -data_dict[self.coord_key][:, 0]
            if self.normal_key in data_dict.keys():
                data_dict[self.normal_key][:, 0] = -data_dict[self.normal_key][:, 0]
            if self.label_key in data_dict.keys():
                if self.label_key == "transform_matrix":
                    data_dict[self.label_key][:3, :3] = np.dot(np.diag([-1, 1, 1]), data_dict[self.label_key][:3, :3])
        if np.random.rand() < self.p:
            if self.coord_key in data_dict.keys():
                data_dict[self.coord_key][:, 1] = -data_dict[self.coord_key][:, 1]
            if self.normal_key in data_dict.keys():
                data_dict[self.normal_key][:, 1] = -data_dict[self.normal_key][:, 1]
            if self.label_key in data_dict.keys():
                if self.label_key == "transform_matrix":
                    data_dict[self.label_key][:3, :3] = np.dot(np.diag([1, -1, 1]), data_dict[self.label_key][:3, :3])
        return data_dict
...
class RandomJitter(object):
    def __init__(self, sigma=0.01, clip=0.05, coord_key="coord", label_key=None):
        assert clip > 0
        self.sigma = sigma
        self.clip = clip
        self.coord_key = coord_key
    ...
    def __call__(self, data_dict):
        if self.coord_key in data_dict.keys():
            jitter = np.clip(
                self.sigma * np.random.randn(data_dict[self.coord_key].shape[0], 3),
                -self.clip,
                self.clip,
            )
            data_dict[self.coord_key] += jitter
        return data_dict
...
class GridSample(object):
    def __init__(
            ...
            input_key="coord",  # 输入坐标字段的名称
            grid_coord_key="grid_coord",  # 输出网格坐标字段的名称
            sampled_index_key="sampled_index",  # sampled_index 的输出字段名称
            inverse_key="inverse",  # inverse 的输出字段名称
            min_coord_key="min_coord",  # min_coord 的输出字段名称
            displacement_key="displacement",  # displacement 的输出字段名称
            normal_key="normal",  # normal 的输出字段名称
            segment_key="segment",  # segment 的输出字段名称
            index_key="index",  # index 的输出字段名称
            index_valid_keys=None,  # 需要切片的字段(手动)
            index_valid_keys_key="index_valid_keys"
    ):
        ...
        self.grid_coord_key = grid_coord_key
        self.sampled_index_key = sampled_index_key
        self.inverse_key = inverse_key
        self.min_coord_key = min_coord_key
    ...
    def __call__(self, data_dict):
        assert self.input_key in data_dict.keys()  # 确保输入坐标字段存在
        scaled_coord = data_dict[self.input_key] / np.array(self.grid_size)
        ...
        if self.mode == "train":  # train mode
            ...
            if self.sampled_index_key in data_dict:
                ...
                idx_unique = np.unique(
                    np.append(idx_unique, data_dict[self.sampled_index_key])
                )
                mask = np.zeros_like(data_dict[self.segment_key]).astype(bool)
                mask[data_dict[self.sampled_index_key]] = True
                data_dict[self.sampled_index_key] = np.where(mask[idx_unique])[0]
            data_dict = index_operator(data_dict, idx_unique,
                                       keys=self.index_valid_keys,
                                       keys_key=self.index_valid_keys_key)
            if self.return_inverse:
                data_dict[self.inverse_key] = np.zeros_like(inverse)
                data_dict[self.inverse_key][idx_sort] = inverse
            if self.return_grid_coord:
                data_dict[self.grid_coord_key] = grid_coord[idx_unique]
                if self.grid_coord_key not in data_dict.get(self.index_valid_keys_key, []):
                    data_dict.setdefault(self.index_valid_keys_key, []).append(self.grid_coord_key)
            if self.return_min_coord:
                data_dict[self.min_coord_key] = min_coord.reshape([1, 3])
            if self.return_displacement:
                ...
                if self.project_displacement:
                    displacement = np.sum(
                        displacement * data_dict[self.normal_key], axis=-1, keepdims=True
                    )
                data_dict[self.displacement_key] = displacement[idx_unique]
                if self.displacement_key not in data_dict.get(self.index_valid_keys_key, []):
                    data_dict.setdefault(self.index_valid_keys_key, []).append(self.displacement_key)
            return data_dict
            ...
            for i in range(count.max()):
                ...
                data_part = index_operator(data_dict, idx_part,
                                           duplicate=True,
                                           keys=self.index_valid_keys,
                                           keys_key=self.index_valid_keys_key)
                data_part[self.index_key] = idx_part
                if self.return_inverse:
                    data_part[self.inverse_key] = np.zeros_like(inverse)
                    data_part[self.inverse_key][idx_sort] = inverse
                if self.return_grid_coord:
                    data_part[self.grid_coord_key] = grid_coord[idx_part]
                    if self.grid_coord_key not in data_part.get(self.index_valid_keys_key, []):
                        data_part.setdefault(self.index_valid_keys_key, []).append(self.grid_coord_key)
                if self.return_min_coord:
                    data_part[self.min_coord_key] = min_coord.reshape([1, 3])
                if self.return_displacement:
                    displacement = (
                            scaled_coord - grid_coord - 0.5
                    )  # [0, 1] -> [-0.5, 0.5] displacement to center
                    ...
                    data_part[self.displacement_key] = displacement[idx_part]
                    if self.displacement_key not in data_part.get(self.index_valid_keys_key, []):
                        data_part.setdefault(self.index_valid_keys_key, []).append(self.displacement_key)
...
class SphereCrop(object):
    ...
    def __call__(self, data_dict):
        point_max = (
            int(self.sample_rate * data_dict[self.coord_key].shape[0])
            if self.sample_rate is not None
            else self.point_max
        )

        if data_dict[self.coord_key].shape[0] > point_max:
            if self.mode == "random":
                center = data_dict[self.coord_key][
                    np.random.randint(data_dict[self.coord_key].shape[0])
                ]
            elif self.mode == "center":
                center = data_dict[self.coord_key][data_dict[self.coord_key].shape[0] // 2]
            else:
                raise NotImplementedError
            idx_crop = np.argsort(np.sum(np.square(data_dict[self.coord_key] - center), 1))[
                       :point_max
                       ]
            data_dict = index_operator(data_dict, idx_crop, keys=self.index_valid_keys, keys_key=self.index_valid_keys_key)
            if self.label_key in data_dict.keys():
                data_dict[self.label_key] = data_dict[self.label_key][idx_crop]
        return data_dict


@TRANSFORMS.register_module()
class ShufflePoint(object):
    def __init__(self, index_valid_keys=None, index_valid_keys_key="index_valid_keys"):
        self.index_valid_keys = index_valid_keys
        self.index_valid_keys_key = index_valid_keys_key
    ...
    def __call__(self, data_dict):
        ...
        data_dict = index_operator(data_dict, shuffle_index, keys=self.index_valid_keys, keys_key=self.index_valid_keys_key)
        return data_dict
...
class CropBoundary(object):
    ...
    def __call__(self, data_dict):
        ...
        data_dict = index_operator(data_dict, mask, keys=self.index_valid_keys, keys_key=self.index_valid_keys_key)
        return data_dict
