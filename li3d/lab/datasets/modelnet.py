...
class ModelNetDataset(Dataset):
    ...
    @staticmethod
    def farthest_point_sampling(xy, n_pts, K):
        """
        Pure PyTorch FPS.
        ----------
        xy   : torch.Tensor (B, N, 2 or 3)  点云坐标
        n_pts: torch.Tensor (B,)            每批实际点数（mask 用）
        K    : torch.Tensor (B,)            要采的个数
        ----------
        return  idx : (B, max(K))  每行是采样下标，后面补 0
        """
        B, N, _ = xy.shape
        device = xy.device
        idx = torch.zeros(B, K.max().item(), dtype=torch.long, device=device)
        # 距离矩阵初始化为无穷
        dist = torch.full((B, N), 1e8, device=device)
        # 随机选第一个点
        farthest = torch.randint(0, N, (B,), device=device)
        # 逐点采样
        for k in range(K.max().item()):
            idx[:, k] = farthest
            # 取出最新点
            centroid = xy[torch.arange(B), farthest, :].view(B, 1, -1)  # (B,1,2/3)
            # 计算到最新点的距离
            dist_now = ((xy - centroid) ** 2).sum(-1)  # (B,N)
            # 只在有效长度内更新
            mask = torch.arange(N, device=device).unsqueeze(0) < n_pts.unsqueeze(1)
            dist = torch.where(mask, torch.min(dist, dist_now), dist)
            # 选最远点
        ...
        if data_name in self.data.keys():
            ...
            if self.num_point is not None:
                if self.uniform_sampling:
                    with torch.no_grad():
                        mask = self.farthest_point_sampling(
