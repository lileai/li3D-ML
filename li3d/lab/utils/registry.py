...
class Registry:
    """字符串到类的映射表，支持通过字符串动态构建实例.

    已注册的类可通过registry.build()方法实例化.
    Example:
    ...
    def get(self, key):
        ...
        Args:
            key (str): The class name in string format e.g.：(mmdet.RssNet).
    ...
    def build(self, *args, **kwargs):
        # 对外接口，用self.build_func构建实例
        return self.build_func(*args, **kwargs, registry=self)

    def _add_children(self, registry):
        """Add children for a registry.
        内部方法：将子注册器按scope加入self._children
