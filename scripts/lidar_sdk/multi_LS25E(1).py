...
class SharedDequeKeepNew:
    """
    多进程安全的队列，满时自动丢弃最老的数据（丢旧留新）
    """
    ...
    def put(self, item, block=True, timeout=None):
        """
        添加数据，如果满，自动丢弃最老的数据
        （注意：block 和 timeout 目前只对 put 生效，内部不阻塞）
        """
        # 无论如何都尝试放入，先腾出空间
        while True:
            try:
                ...
                # 队列满，尝试移除最老的数据
                try:
                    self.queue.get_nowait()  # 非阻塞获取并丢弃
                except Empty:
                    # 理论上不会发生，以防万一
                    pass
    ...
    def get(self, block=True, timeout=None):
        ...
        try:
            ...
            raise Empty("SharedDequeKeepNew is empty")

    def qsize(self):
        """返回当前大小"""
        # 注意：multiprocessing.Queue.qsize() 在某些系统可能不支持
        try:
            return self.queue.qsize()
        except NotImplementedError:
            # 可以通过其他方式估算，或返回 None
            raise NotImplementedError("qsize not supported on this platform")
...
class UdpListener(Process):
    def __init__(self, status_dict, config_dict, data_queue):
        super().__init__()
...
class MultiLS25E(Process):
    ...
    def handleSingleEcho(self, key_ip, data):
        self.dict_ip_decoding_config[key_ip]["PointIntervalT_ns"] = (self.dict_ip_decoding_config[key_ip][
                                                                         "allTimestamp"] -
                                                                     self.dict_ip_decoding_config[key_ip][
                                                                         "lastAllTimestamp"]) / 149.0
        for i in range(0, 1192, 8):
            ...
            if self.dict_ip_decoding_config[key_ip]["count"] == self.lidar_NUMS:
                ...
                if self.dict_ip_decoding_config[key_ip]["m_StackFrame"] > 1:
                    self.dict_ip_decoding_config[key_ip]["tempPointCloud"] = self.dict_ip_decoding_config[key_ip][
                        "LidarPerFrameDatePrt_Get"].copy()
                    self.dict_ip_decoding_config[key_ip]["LidarPerFrameDatePrt_Get"] = \
                    self.dict_ip_decoding_config[key_ip]["PointCloudLastData"] + self.dict_ip_decoding_config[key_ip][
                        "LidarPerFrameDatePrt_Get"]
                    self.dict_ip_decoding_config[key_ip]["PointCloudLastData"] = self.dict_ip_decoding_config[key_ip][
                        "tempPointCloud"]
            ...
            if self.dict_ip_decoding_config[key_ip]["count"] == 0:
                ...
                self.dict_ip_decoding_config[key_ip]["m_DistanceIsNotZero"] = self.dict_ip_decoding_config[key_ip][
                    "m_DistanceIsNotZero"] if self.dict_ip_decoding_config[key_ip]["m_DistanceIsNotZero"] > 30 else (
                    self.dict_ip_decoding_config[key_ip]["m_DistanceIsNotZero"] + 1 if abs(tDistance - 0) > 1e-8 else
                    self.dict_ip_decoding_config[key_ip]["m_DistanceIsNotZero"])
                if tDistance <= 0:
                    continue
                ...
                if tChannelID >= len(
                        self.dict_ip_decoding_config[key_ip]["mLidaFilterParamDisplayValue"]['mChannelVector']):
                    print("id无效")
                    continue
                if self.dict_ip_decoding_config[key_ip]["mLidaFilterParamDisplayValue"]['mChannelVector'][
                    tChannelID] == 0:
                    print("通道未启用")
                    continue

                if not self.dict_ip_decoding_config[key_ip]["mLidaFilterParamDisplayValue"][
                           'mMin_Distance'] <= tDistance <= \
                       self.dict_ip_decoding_config[key_ip]["mLidaFilterParamDisplayValue"]['mMax_Distance']:
                    print(f"距离超出范围")
                    continue
                if not self.dict_ip_decoding_config[key_ip]["mLidaFilterParamDisplayValue"][
                           'mMin_Intensity'] <= intensity <= \
                ...
                m_DataT = LidarData(
                    ...
                    Y=m_point['y1'],
                    Z=m_point['z1'],
                    Mtimestamp_nsce=self.dict_ip_decoding_config[key_ip]["allTimestamp"] -
                                    self.dict_ip_decoding_config[key_ip]["PointIntervalT_ns"] * (148 - i // 8)
                )
    ...
    def handleDoubleEcho(self, key_ip, data):
        ...
        for i in range(0, 1188, 12):
            ...
            if self.dict_ip_decoding_config[key_ip]["count"] == 0:
                ...
                tDistance_2 = ((data[i + 8] << 16) + (data[i + 9] << 8) + data[i + 10]) * \
                              self.dict_ip_decoding_config[key_ip]["m_fDistanceAcc"]
                ...
                m_DataT = LidarData(
                    ...
                    Z=m_point['z1'],
                    Mtimestamp_nsce=self.dict_ip_decoding_config[key_ip]["allTimestamp"] -
                                    self.dict_ip_decoding_config[key_ip]["PointIntervalT_ns"] * (98 - i // 12)
                )
                self.dict_ip_decoding_config[key_ip]["LidarPerFrameDatePrt_Get"].append(m_DataT)
    ...
    def XYZ_calculate(self, key_ip, tChannelID, fAngle_H, fAngle_V, tDistance, tDistance2):
        if self.dict_ip_decoding_config[key_ip]["lidar_EchoModel"] in [0x11, 0x12]:
            point = {
                'x1': tDistance * self.dict_ip_decoding_config[key_ip]["cosAngleValue"][negative2positive(fAngle_V)] *
                      self.dict_ip_decoding_config[key_ip]["sinAngleValue"][
                          negative2positive(fAngle_H)],
                'y1': tDistance * self.dict_ip_decoding_config[key_ip]["cosAngleValue"][negative2positive(fAngle_V)] *
                      self.dict_ip_decoding_config[key_ip]["cosAngleValue"][
                          negative2positive(fAngle_H)],
            ...
            if tDistance2 > 0:
                point['x2'] = tDistance2 * self.dict_ip_decoding_config[key_ip]["cosAngleValue"][
                    negative2positive(fAngle_V)] * self.dict_ip_decoding_config[key_ip]["sinAngleValue"][
                                  negative2positive(fAngle_H)]
                point['y2'] = tDistance2 * self.dict_ip_decoding_config[key_ip]["cosAngleValue"][
                    negative2positive(fAngle_V)] * self.dict_ip_decoding_config[key_ip]["cosAngleValue"][
                                  negative2positive(fAngle_H)]
                point['z2'] = tDistance2 * self.dict_ip_decoding_config[key_ip]["sinAngleValue"][
                    negative2positive(fAngle_V)]
        else:
            point = {
                'x1': 0,
                'y1': 0,
            ...
            fAngle_R0 = float(self.dict_ip_decoding_config[key_ip]["cosAngleValue"][negative2positive(30)] *
                              self.dict_ip_decoding_config[key_ip]["cosAngleValue"][
            ...
            fSinV_angle = float(2 * fAngle_R0 * self.dict_ip_decoding_config[key_ip]["sinAngleValue"][
                negative2positive(fGalvanometrtAngle)] + \
                                self.dict_ip_decoding_config[key_ip]["sinAngleValue"][
                                    negative2positive(fPutTheMirrorOffAngle)])
            fCosV_angle = float(np.sqrt(1 - fSinV_angle * fSinV_angle))

            fSinCite = float((2 * fAngle_R0 * self.dict_ip_decoding_config[key_ip]["cosAngleValue"][
                negative2positive(fGalvanometrtAngle)] *
            ...
            fSinCite_H = float(
                self.dict_ip_decoding_config[key_ip]["sinAngleValue"][negative2positive(fAngle_H)] * fCosCite +
                self.dict_ip_decoding_config[key_ip]["cosAngleValue"][
                    negative2positive(fAngle_H)] * fSinCite)
            fCosCite_H = float(
                self.dict_ip_decoding_config[key_ip]["cosAngleValue"][negative2positive(fAngle_H)] * fCosCite -
                self.dict_ip_decoding_config[key_ip]["sinAngleValue"][
                    negative2positive(fAngle_H)] * fSinCite)
    ...
    def send_lidar_data(self, key_ip):
        try:
            print("正在写入雷达信息")
            # 检查距离数据是否有效
            if self.dict_ip_decoding_config[key_ip]["m_DistanceIsNotZero"] < self.dict_ip_decoding_config[key_ip][
                "m_DistanceThreshold"]:
    ...
    def create_process(self):
        for lidar_config in self.list_lidar_config:
            udp_listener = UdpListener(self.status_dict, lidar_config, self.udp_data_queue)
    ...
    def run(self):
        try:
            ...
            while True:
                ...
                # ----------------------------------------------------------------
                try:
                    self.UDP_Decode(key_ip=addr[0], data=data)
                except:
...
if __name__ == "__main__":
    config = {
        "lidar_info_list": [
            {"lidar_ip": "192.168.0.200",
             "calibration_matrix": [[0.966323, -0.255392, -0.031539, 0], [0.255392, 0.936786, 0.239181, 0],
                                    [-0.031539, -0.239181, 0.970463, 0], [0, 0, 0, 1]]}
        ],
    ...
    points_queue = SharedDequeKeepNew(maxlen=3000)
