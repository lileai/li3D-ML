# ------------------------------------------------------------------------
# -*- coding: utf-8 -*-
# @Time    : 2024/taipu_main_side/28 13:20
# @Author  : Leii
# @File    : filter.py
# @Code instructions: 
# ------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    intensity_process = intensity[np.where(point[:, 2] <= z_avg)]
    intensity_no_process = intensity[np.where(point[:, 2] > z_avg)]
    # 平均高度往上是不会出现地面的
    ...
    intensity_high = intensity_process[np.where(surface_curvature > angle_raid)]

    point_low = point_process[np.where(surface_curvature <= angle_raid)]
    intensity_low = intensity_process[np.where(surface_curvature <= angle_raid)]
    # 区分立面点和地面点
    point_fac = point[np.where(abs(nz) < 0.2)]
    intensity_fac = intensity[np.where(abs(nz) < 0.2)]

    point_pla = point[np.where(abs(nz) > 0.99)]
    intensity_pla = intensity[np.where(abs(nz) > 0.99)]
    ...
    intensity_no_ground = np.concatenate((intensity_high, intensity_fac), axis=0)
    intensity_no_ground_fnl = np.concatenate((intensity_no_ground, intensity_no_process), axis=0)
    unique_points, index = np.unique(pcd_no_ground_finl.points, return_index=True, axis=0)
    intensity_no_ground_fnl = intensity_no_ground_fnl[index]
    pcd_no_ground_finl.points = o3d.utility.Vector3dVector(unique_points)
    pcd_t_no_ground_fnl = o3d.t.geometry.PointCloud()
    dtype = o3d.core.float32
    pcd_t_no_ground_fnl.point["positions"] = o3d.core.Tensor(unique_points, dtype)
    pcd_t_no_ground_fnl.point["intensity"] = o3d.core.Tensor(intensity_no_ground_fnl, dtype)
    ...
    file_t_name = f'{path}/{os.path.splitext(filename)[0]}_curvature_t_pcd_{rate}.pcd'
    # o3d.io.write_point_cloud(file_name, pcd_no_ground_finl)
    o3d.io.write_point_cloud(file_name, pcd_no_ground_finl)
    o3d.t.io.write_point_cloud(file_t_name, pcd_t_no_ground_fnl)
    pcd_finl_test = o3d.t.io.read_point_cloud(file_t_name)
    ...
    pcd_finl_test.paint_uniform_color([0, 0, 1])
    ...
    vis.add_geometry("test", pcd_finl_test)
    app.add_window(vis)
    app.run()
    # vison(pcd_finl_test)
