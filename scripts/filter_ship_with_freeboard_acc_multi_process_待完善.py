...
def apply_rotation(points, center_points, density, points_thresh, density_thresh):
    """
...
def gaussian_approximation(u_list, sigma=0.25, distinguishing_coefficient=10, A=1):
    if len(u_list) == 0:
        return None
    select_value = list(
        range(int((min(u_list) - 1) * distinguishing_coefficient), int((max(u_list) + 1) * distinguishing_coefficient)))
...
def find_most_frequent_value(arr, precision=1, n=1, threshold=0.15):
    # 将数组中的每个元素保留到指定的小数位数
    ...
    # 如果最大的组的大小小于 n / 5，返回 None
    if len(largest_group) < n / 5:
        print("true")
        return None
...
class LidarFunc(object):
    ...
    def freeboard_calculation(self, points, water_level=None):
        ...
        if len(points) > self.parser_dict['filter_thresh']:
            nb_neighbors = self.parser_dict['nb_neighbors'] * density
            std_ratio = self.parser_dict['std_ratio'] * roughness
        ...
        for divide_rotation_center in divide_rotation_center_list:
            ...
            dvide_x_length = divide_rotation_center[:, 0].max() - divide_rotation_center[:, 0].min()
            x_ratio = max(1, dvide_x_length)
            y_top_index = np.argsort(direction * np.abs(y_coords))[
                          :max(1, int(self.parser_dict["nums"] * roughness * x_ratio))]
            # 对其z轴大小排序
...
def process_single_pcd_file(args):
    """处理单个PCD文件的函数，用于多进程"""
    pcd_file, parser_dict = args
    try:
        # 设置随机种子以保证结果一致性
        o3d.utility.random.seed(42)
        ...
        return {
            ...
            'density': density,
            'roughness': roughness,
            'freeboard_upper_bound': freeboard_upper_bound,
            'error': None
        }
    except Exception as e:
        # 捕获异常并返回错误信息
        error_info = f"Error processing {pcd_file}: {str(e)}\n{traceback.format_exc()}"
        print(error_info)  # 打印到控制台以便调试
        return {
            'file': pcd_file,
            'x_center': None,
            'density': None,
...
def process_directory(dir_path, parser_dict, output_excel_path, png_dir_path, folder_name, num_processes):
    """处理单个目录下的所有PCD文件"""
    detail_dir = os.path.join(dir_path, "detail")
    current_path = os.path.join(detail_dir, "deck_pcd")
    if not os.path.exists(current_path):
        print(f"目录不存在: {current_path}")
        return

    pcd_files = glob.glob(os.path.join(current_path, '*.pcd'))
    if not pcd_files:
        print(f"目录 {current_path} 中没有找到PCD文件")
        return

    frame_freeboard_upper_bound = []

    # 准备多进程参数
    process_args = [(pcd_file, parser_dict) for pcd_file in pcd_files]

    # 使用多进程处理
    with mp.Pool(processes=num_processes) as pool:
        # 使用tqdm显示进度条
        results = list(tqdm(pool.imap(process_single_pcd_file, process_args),
                            total=len(pcd_files), desc=f"Processing {os.path.basename(dir_path)}"))

    # 收集结果
    for result in results:
        if result['freeboard_upper_bound'] is not None:
            frame_freeboard_upper_bound.append(result['freeboard_upper_bound'])
        # 可以在这里处理错误信息 result['error']
    ...
    png_path = os.path.join(png_dir_path, folder_name, dir_name)
    ...
    print(f"目录 {dir_name} 处理完成，峰值: {steady_state_freeboard_upper_bound}")


if __name__ == '__main__':
    # 可以通过命令行参数传递进程数
    parser = argparse.ArgumentParser(description='Process point cloud data with multiprocessing.')
    parser.add_argument('--processes', type=int, default=mp.cpu_count(), help='Number of processes to use')
    args = parser.parse_args()
    num_processes = args.processes

    # 固定路径和配置
    directory_path = r"../data/Taipu-main/20250830"
    output_excel_path = './table/select.xlsx'
    png_dir_path = './table'
    folder_name = ".select"
    ...
    os.makedirs(os.path.join(png_dir_path, folder_name), exist_ok=True)
    ...
    start_index = dirs.index(last_dir) + 1 if last_dir and last_dir in dirs else 0

    # 遍历并处理每个目录
    for dir in dirs[start_index:]:
        dir_full_path = os.path.join(directory_path, dir)
        print(f"开始处理目录: {dir}")
        process_directory(dir_full_path, parser_dict, output_excel_path, png_dir_path, folder_name, num_processes)
